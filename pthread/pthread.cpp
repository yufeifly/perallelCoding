...
void *pfunc(void* arg)
{
	...
	for (int j = 1 + (long)arg ; j < myMat->rows - 1; j += nthread) {
		...
		{
			*output++ = saturate_cast<uchar>(current[i] * 9 - previous[i] - next[i]
			                                 - current[i - nChannels] - current[i + nChannels] * conv[5]
			                                 - previous[i - nChannels] - previous[i + nChannels]
			                                 - next[i - nChannels] - next[i + nChannels]);
		}
	}
